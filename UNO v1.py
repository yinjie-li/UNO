...
def afficher_carte(indice):
    print(dict_contenu[indice], dict_couleur[indice])
...
def afficher_main(main_du_joueur):
    for i in range(len(main_du_joueur)):
        print('Carte', i + 1, ':')
        afficher_carte(main_du_joueur[i])
...
def jouer(indice, main_du_joueur):
    if indice == 0:
        ...
        while indice > len(main_du_joueur) or \
            ...
            int(input('Quelle carte voulez-vous jouer? Ou tapez 0 pour piocher une carte. '))
        if indice == 0:
            main_du_joueur.append(paquet.pop(-1))
        elif verfication(main_du_joueur[indice - 1]):
            pile.append(main_du_joueur[indice - 1])
...
def tour(joueur):
    global pioche
    global sens
    afficher_main(joueur_main[joueur])
    if pioche != 0:
        i = True
        for j in joueur_main[joueur]:
            if dict_contenu[joueur_main[joueur][j]] == '+4':
                i = i and True
            elif dict_contenu[joueur_main[joueur][j]] == '+2':
                i = i and verfication(joueur_main[joueur][j])
            else:
                i = i and False
        if not i:
            piocher(joueur_main[joueur], pioche)
            pioche = 0
        joueur = joueur = joueur + sens
    else:
        carte_a_jouer = int(
            input('Quelle carte voulez-vous jouer? Ou tapez 0 pour piocher une carte. '))
        jouer(carte_a_jouer, joueur_main[joueur])
        joueur += sens
        if pile != ['test']:
